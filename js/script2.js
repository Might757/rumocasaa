...
function initMap() {
    ...
    let url = 'db/adultos/gerirOfertaFormativaAdultos.php?getEscolasMap=1';
...
function addBackgroundColorByTipoCurso(tipo){
 ...
 return "";
    ...
    $("#pesquisar").click(function() {
        ...
        url = 'db/adultos/gerirOfertaFormativaAdultos.php?getEscolasCursos='+concelho+'&idEscola='+escola+'&idTipoCurso='+tipoCurso+'&idEnsino='+ensino+'&nomeEscola='+escolaNome;
...
$( document ).ready(function() {
    ...
    function info(idEscola, idCurso){
        var num = 0;
        let cursos = "";
        let url = 'db/adultos/gerirOfertaFormativaAdultos.php?getEscolaCursoInfo='+idEscola+'&idCurso='+idCurso;
        $.post(url, function(data)
        {   
            let arr = JSON.parse(data);
    ...
    {
        let url = 'db/adultos/gerirOfertaFormativaAdultos.php?getConcelhos=1';
        $.post(url, function(data)
        {      
            let arr = JSON.parse(data);
            for(let i = 0; i<arr.length; i++)
    ...
    {       
        ...
        if($("#concelho").val() == "todosConcelhos"){
            url = 'db/adultos/gerirOfertaFormativaAdultos.php?getEscolas=1';
        }else{
            url = 'db/adultos/gerirOfertaFormativaAdultos.php?getEscolasByConcelho='+$("#concelho").val();
        }
        $.post(url, function(data)
        {      
            let arr = JSON.parse(data);
    ...
    { 
        ...
        if($('#escola').val() == "todasEscolas"){
            ...
            if($('#concelho').val() == 1 || $('#concelho').val() == "todosConcelhos"){
                $('#ensino').append("<option value='1'>Conclusão do 4ºano (Educação e Formação de Adultos escolar)</option>");
                $('#ensino').append("<option value='2'>Conclusão do 6ºano (Educação e Formação de Adultos escolar)</option>"); 
                $('#ensino').append("<option value='3'>Conclusão do 9ºano (Educação e Formação de Adultos escolar)</option>");
                $('#ensino').append("<option value='4'>Conclusão do 12ª ano (Educação e Formação de Adultos escolar)</option>");
                $('#ensino').append("<option value='5'>Conclusão 4º, 6º e 9º ano (Reconhecimento de Validação e Certificação de Competências)</option>");
                $('#ensino').append("<option value='6'>Conclusão 12º ano (Reconhecimento de Validação e Certificação de Competências)</option>");
                $('#ensino').append("<option value='8'>Formações Modulares (Vida ativa, UFCD)</option>");
                $('#ensino').append("<option value='7'>Outras</option>");
          
            }else{
                if($('#escola').val() == 3){
                    $('#ensino').append("<option value='4'>EFA escolar (Conclusão do 12ª ano)</option>");
                    $('#ensino').append("<option value='5'>RVCC escolar básico</option>");
                    $('#ensino').append("<option value='7'>Outras</option>");
                    $('#ensino').val("default");
                }else if($('#escola').val() == 5){
                    $('#ensino').append("<option value='1'>Conclusão do 4ºano (EFA escolar)</option>");
                    $('#ensino').append("<option value='2'>Conclusão do 6ºano (EFA escolar)</option>"); 
                    $('#ensino').append("<option value='3'>Conclusão do 9ºano (EFA escolar)</option>");
                    $('#ensino').append("<option value='4'>Conclusão do 12ª ano (EFA escolar)</option>");
                    $('#ensino').val("default");
                }else if($('#escola').val() == 6){
                    $('#ensino').append("<option value='1'>Conclusão do 4ºano (EFA escolar)</option>");
                    $('#ensino').append("<option value='2'>Conclusão do 6ºano (EFA escolar)</option>"); 
                    $('#ensino').append("<option value='3'>Conclusão do 9ºano (EFA escolar)</option>");
                    $('#ensino').append("<option value='4'>Conclusão do 12ª ano (EFA escolar)</option>");
                    $('#ensino').append("<option value='5'>RVCC escolar</option>");    
                    $('#ensino').val("default");
                    console.log($('#ensino').val());
                }
            }
        }else{
            if($('#escola').val() == 3){
                ...
                $('#ensino').append("<option value='1'>EFA escolar (Conclusão do 4ºano)</option>");
                $('#ensino').append("<option value='2'>EFA escolar (Conclusão do 6ºano)</option>"); 
                $('#ensino').append("<option value='3'>EFA escolar (Conclusão do 9ºano)</option>");
                $('#ensino').append("<option value='4'>EFA escolar (Conclusão do 12ª ano)</option>");
            }else if($('#escola').val() == 6){
                $('#ensino').append("<option value='1'>EFA escolar (Conclusão do 4ºano)</option>");
                $('#ensino').append("<option value='2'>EFA escolar (Conclusão do 6ºano)</option>"); 
    ...
    function getTipoDeCurso() //MUDAR ISTO !!!! IMPORTANTE
    {
        console.log($('#ensino').val() + "ISTO É O ENSINO!");
        $('#tipologia').find('option').not(':first').remove();
        if($('#escola').val() == 3 && $('#ensino').val() == 4){
            $('#tipologia').append(`<option value="5">Escolar</option>`);
            $('#tipologia').val("5");
            
        }else if($('#escola').val() == 3 && $('#ensino').val() == 5){
            $('#tipologia').append(`<option value="6">Escolar</option>`); 
            $('#tipologia').val("6");

        }else if($('#escola').val() == 3 && $('#ensino').val() == 5){
            $('#tipologia').append(`<option value="9">Outras</option>`); 
            $('#tipologia').val("9");

        }else{
            let url = 'db/adultos/gerirOfertaFormativaAdultos.php?getTiposCursos='+$("#ensino").val();
            $.post(url, function(data)
            {      
                console.log(data)
                let arr = JSON.parse(data);
    ...
    function getDescricao(){
        ...
        let url = 'db/adultos/gerirOfertaFormativaAdultos.php?getTipoCursoDescricao='+$("#tipologia").val();
        $.post(url, function(data)
        {      
            let arr = JSON.parse(data);
